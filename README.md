...
- **`results/`**: Outputs generated by the code.
  - Parameter CSVs (`bb_params.csv`, etc.).
  - Error analysis reports evaluating empirical accuracy.
  - Wavefunction expansion parameters.
- **`qho/`** (Legacy):
...
2. For each sector ($b\bar{b}$, $c\bar{c}$, $b\bar{c}$), checks if optimized parameters exist in `results/`. If not, it runs the global optimization fitter.
3. Generates the full basis of $S, P,$ and $D$ wave states using GEM.
4. Computes relativistic perturbative corrections (Spin-Orbit, Tensor).
5. Mixes the $1^3S_1$ and $1^3D_1$ states.
6. Computes decay rates (Leptonic and Two-Photon widths) via the Hypervirial Theorem.
7. Outputs detailed, tabulated comparison charts directly to the console and exports data to the `results/` folder.
